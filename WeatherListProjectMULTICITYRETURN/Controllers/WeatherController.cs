        ...
        public async Task<IActionResult> City(List<string> myCities)
        {
...

            {
                try
                {
                    ...
                    Dictionary<string, float> cityWeather = new Dictionary<string, float>();
                    foreach (string city in listReturns)
                    {
                        var rawWeather = JsonSerializer.Deserialize<OpenWeatherResponse>(city, options);
                        cityWeather.Add(rawWeather.Name, rawWeather.Main.Temp);
                    }
